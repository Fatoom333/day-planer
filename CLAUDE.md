...
- Пары берутся из Google Calendar через Google Identity Services (token flow, scopes `https://www.googleapis.com/auth/calendar.events.readonly` — обязательный, и `https://www.googleapis.com/auth/calendar.calendarlist.readonly` — по желанию, для выбора календаря из списка; добавлен с согласия пользователя 2026-09-26). Названия календарей живут только в памяти, в хранилище — одни ID. Токен хранится только в памяти. Client ID лежит в `src/config.js`. Он публичный по природе (не секрет), а защиту даёт список разрешённых origin в Google Cloud. Client secret не нужен и в репо не попадает никогда.
